...
function renderShoppingList() {
  ...
  // set up a copy of the store's items in a local variable that we will reassign to a new version if searching occurs
  let searchedItems = STORE.items;
  //if searchItem > 0 (there is an input) we want to see only items that contain the input//
  if (STORE.searchItem) {
    filteredItems = searchedItems.filter(function(item){ 
       return item.name.includes(STORE.searchItem);
    });
    generateShoppingItemsString(searchedItems);
